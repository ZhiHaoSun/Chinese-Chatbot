...
max_token_id = wordToken.load_file_list([data_folder + '/question',  data_folder + '/answer'], min_freq)
...
def get_train_set():
    ...
    train_set = []
    with open(data_folder + '/question', 'r') as question_file:
        with open(data_folder + '/answer', 'r') as answer_file:
            while True:
...
def train():
    ...
    with tf.Session() as sess:
        ...
        if continue_training:
            saver.restore(sess, './model/' + data_folder)
        else:
            # 全部变量初始化
            sess.run(tf.global_variables_initializer())

        # 训练很多次迭代，每隔4次打印一次loss，可以看情况直接ctrl+c停止
        previous_losses = []
        for step in range(1000):
            ...
            if step % 4 == 0:
                ...
                # 模型持久化
                saver.save(sess, './model/' + data_folder)
